 ...
 * 组成：
 *   1) 顶部"开启新对话"按钮；
 *   2) 会话列表：标题（当前直接显示 thread_id）+ 更新时间；
 *   3) 悬浮交互：鼠标悬在会话上时右侧出现"..."按钮，点击弹出菜单——只有"删除"一项；
 *   4) 删除确认弹窗：点击"删除"后先确认，再调后端清空该会话的数据库数据；
 *   5) 左下方：浅色/深色主题切换——显示"当前主题"（浅色=小太阳图标/浅色模式，
 *      深色=月亮图标/深色模式），点击即切换。
 *
 * 交互约定：streaming=true（正在流式回答）时禁用切换/删除（点击被忽略）。
 */
import { useEffect, useState } from 'react'
import type { MouseEvent } from 'react'

import type { SessionInfo, Theme } from '../types'
...
export default function Sidebar({
  ...
  return (
    <aside className="sidebar">
      {/* 顶部：开启新对话 */}
      <button type="button" className="new-chat-btn" onClick={onNewChat} disabled={streaming}>
        <PlusIcon />
        开启新对话
      </button>
      ...
      {/* 左下方：浅色/深色主题切换（显示当前主题，点击切换；选择由 App 持久化到 localStorage） */}
